...
- (id)initWithPlistRepresentation:(NSDictionary *)aDictionary;
{
   ...
   NSData *someData = UtilityDataWithResizedCGImage(
      [imageRep CGImage],
      &imageWidth,
      &imageHeight,
      NO);

   if(nil == imageRep || 0 == imageWidth || 0 == imageHeight)
   {
      self = nil;
   }
   else
   {
      self = [self initWithData:someData
            width:imageWidth
            height:imageHeight];
   }
...
/////////////////////////////////////////////////////////////////
//
- (NSMutableData *)mutableImageData
{
   NSMutableData *result = [[imageData mutableCopy]
      autorelease];
...
- (NSImage *)image;
{
   ...
   NSMutableData *mutableImageData = self.mutableImageData;
...
{
   if(0 != name && nil != self.imageData)
   {  // Update the texture in GPU memory
      const unsigned char *bytesPtr = 
         (const unsigned char *)[self.imageData bytes];
...
{
   NSMutableData *mutableImageData = self.mutableImageData;
   NSAssert(nil != mutableImageData, 
      @"No available image data");
      
   unsigned char *bytesPtr = 
      (unsigned char *)[mutableImageData mutableBytes];
   const GLint x = imageWidth * MIN(MAX(aPosition.x, 0.0f), 1.0f);
   const GLint y = imageHeight * MIN(MAX(aPosition.y, 0.0f), 1.0f);
   const GLfloat radiusSquared = (aRadius * aRadius);
   
   // Fill radius cenered on aPosition with RGB color components,
   // values, and blend edges
   for(int j = MAX(0, y - aRadius); j < MIN(imageHeight, y + aRadius); 
      j++)
   {
      for(int i = MAX(0, x - aRadius); 
         i < MIN(imageWidth, x + aRadius); i++)
      {
         ...
         unsigned char *localBytesPtr = bytesPtr + 
            ((j * imageWidth + i) * 4);
            
         const float alphaR = alpha * values.r;
         localBytesPtr[0] = (localBytesPtr[0] * (1.0f - alpha)) + 
            (255 * alphaR);
         const float alphaG = alpha * values.g;
         localBytesPtr[1] = (localBytesPtr[1] * (1.0f - alpha)) + 
            (255 * alphaG);
         const float alphaB = alpha * values.b;
         localBytesPtr[2] = (localBytesPtr[2] * (1.0f - alpha)) + 
            (255 * alphaB);
      }
   }
   
   [self updateGPUImageData];
}
