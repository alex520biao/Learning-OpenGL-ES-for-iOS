//
//  OpenGLES_Ch7_3ViewController.m
//  OpenGLES_Ch7_3
//

#import "OpenGLES_Ch7_3ViewController.h"
...
@interface OpenGLES_Ch7_3ViewController ()
...
@implementation OpenGLES_Ch7_3ViewController
...
- (void)viewDidLoad
{
   ...
   NSString *modelsPath = [[NSBundle mainBundle] pathForResource:
      @"armatureSkin" ofType:@"modelplist"];
   if(nil != modelsPath)
   {
      self.modelManager = 
   ...
   self.tube = [self.modelManager 
      modelNamed:@"tube"];
   NSAssert(nil != self.tube, 
      @"Failed to load tube model");
   ...
   [self.tube automaticallySkinRigidWithJoints:
      baseEffect.jointsArray];
...
- (IBAction)takeRigidSkinFrom:(UISwitch *)sender
{
   ...
   {
      [self.tube automaticallySkinSmoothWithJoints:
         baseEffect.jointsArray];
   }
}
